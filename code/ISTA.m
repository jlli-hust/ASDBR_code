...
%%
function [xhk,err]=ISTA(y,A,lambda,iter,w)
if nargin<4;
...
for i=1:iter;
    x=xhk;
     ck=xhk-tk*A'*(A*xhk-y);  %计算c_k
     xhk=(max(abs(ck)-alp*w,0)).*sign(ck);  %更新x_k,进入下次迭代
     err(i+1)=norm(A*xhk-y,2);
     if norm(xhk-x,2)<1e-6;  %跳出循环
         err=err(2:end);
         break;
     end
end
